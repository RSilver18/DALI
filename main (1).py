...
def chkBeer():
    if(var8.get()==1):
        ...
        E_Beer.set("0")
#============================== PATENT =================================#
lblName = Label(Name_F,text="\t\t\tMADE BY RUCHITA NAIR, IPSIT MITTAL and AAKAR GUPTA\t\t\t\t\t",justify=CENTER,fg="white",bd=2,bg="black",font=("arial",16,"bold"))
lblName.grid()
#============================== DRINKS =================================#
...
connection_obj=pymysql.connect(host="localhost",user="root",passwd="")
...
cur.execute("Create database FoodProject12I")
cur.execute("use FoodProject12I")
name=Customer.get()
cur.execute("create table if not exists points(Customer_Name varchar(40), Total_Points int(30))")

cur.execute("select * from points")
for i in cur:
    if i[0]==name:
        new_pts=(int(i[1])+points_earned,)
        s="Update points Total_Points=%s"
        cur.execute(s,new_pts)
        total_points=new_pts
    else:
        s="Insert into point values(%s,%s)"
        val=(name,points_earned)
        cur.execute(s,val)
...
#=========================================================================#
    ...
    txtReceipt.insert(END,"\n")
    txtReceipt.insert(END,f"You have earned {points_earned} points for this purchse.\n")
    txtReceipt.insert(END,f"You have total {total_points} on your account now.")
...
#=========================== POINTS ADDITION ==========================#
PointsInfo1 = Label(Cal_F,font = ("arial",14,"bold"),text="For every 10 Rupees you spend, you earn a point.",bg="powder blue",fg="black")
PointsInfo2 = Label(Cal_F,font = ("arial",14,"bold"),text="For every 100 Points you earn, you unlock new and exciting gifts",bg="powder blue",fg="black")
PointsInfo3 = Label(Cal_F,font = ("arial",14,"bold"),text="rewards, which can be claimed at the billing counter.",bg="powder blue",fg="black")
empty_label = Label(Cal_F,font = ("arial",14,"bold"),text="",bg="cadet blue",fg="black")
PointsInfo1.grid(row=1,column=1)
empty_label.grid(row=2,column=1)
PointsInfo2.grid(row=3,column=1)
PointsInfo3.grid(row=4,column=1)
